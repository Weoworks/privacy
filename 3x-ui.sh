...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}4.${plain} Create SSL Certificate"
    ...
    4)
        ...
        # 输入域名（用于证书CN）
        local self_domain=""
        read -rp "Enter domain or IP for certificate (CN): " self_domain
        self_domain="${self_domain// /}"

        if [[ -z "$self_domain" ]]; then
            echo -e "${red}Domain/IP cannot be empty.${plain}"
            SSL_HOST="${server_ip}"
            return 1
        fi
        ...
        local keyFile="${certDir}/privkey.pem"
        local certFile="${certDir}/fullchain.pem"

        # 生成 4096 位 RSA 私钥 + 10年有效证书
        ...
        echo -e "${green}Self-signed certificate created:${plain}"
        echo -e "  Cert: ${certFile}"
        echo -e "  Key:  ${keyFile}"
        echo -e "${yellow}Valid for 10 years (3650 days)${plain}"

        # 设置到 x-ui
        ${xui_folder}/x-ui cert -webCert "${certFile}" -webCertKey "${keyFile}"

        # 重启面板
        systemctl restart x-ui 2>/dev/null || rc-service x-ui restart 2>/dev/null

        SSL_HOST="${self_domain}"

        echo -e "${green}✓ Self-signed certificate applied successfully!${plain}"
        ;;
    *)
