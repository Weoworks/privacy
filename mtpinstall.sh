...
echo "$(tput setaf 3)Warning!$(tput sgr 0) Do not use special characters like \" , ' , $ or... for username"
while true; do
	echo "现在告诉我一个用户名，用户名用于命名secret："
	read -r -e -i "MTSecret$COUNTER" USERNAME
	echo "您想手动设置密钥还是我创建一个随机密钥？"
	echo "   1) 手动设置"
	echo "   2) 自动生成"
	read -r -p "请选择(建议直接回车) [1-2]: " -e -i 2 OPTION
	case $OPTION in
	1)
	...
	*)
	...
	SECRETS+="$SECRETTEMP , "
	#Setup limiter
	read -r -p "是否要限制连接到此secret的用户？(y/n，默认为否) " -e -i "n" OPTION
	OPTION="$(echo $OPTION | tr '[A-Z]' '[a-z]')"
	case $OPTION in
	'y')
		read -r -p "您想要连接多少用户到这个secret？ " OPTION
...
#Now setup the tag
read -r -p "您想要设置广告标签吗？(y/n，不懂填n) " -e -i "n" OPTION
if [[ "$OPTION" == "y" || "$OPTION" == "Y" ]]; then
...
#Change host mask
read -r -p "选择DPI认为您正在访问的主机 (TLS_DOMAIN)（默认即可）: " -e -i "www.cloudflare.com" TLS_DOMAIN
#Now lets install
